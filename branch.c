...
Branch_Balance(Bank *bank, BranchID branchID, AccountAmount *balance)
{
  ...
  //   pthread_mutex_lock(&branch->accounts[i].lock);
  // }

  // pthread_mutex_lock(&branch->lock);
  pthread_mutex_lock(&branch->lock);
  // for(int i = 0; i < branch->accounts)
  *balance = branch->balance;  Y;
  pthread_mutex_unlock(&branch->lock);
  // pthread_mutex_unlock(&branch->lock);

  // for(int i = 0; i < branch->numberAccounts; i++){
  //   pthread_mutex_unlock(&branch->accounts[i].lock);
  // }
...
Branch_Validate(Bank *bank, BranchID branchID)
{
  ...
  for (int a = 0; a < branch->numberAccounts; a++) {
    // pthread_mutex_lock(&branch->accounts[a].lock);
    total += branch->accounts[a].balance;
    // pthread_mutex_unlock(&branch->accounts[a].lock);
  }
...
Branch_Compare(Branch *branch1, Branch *branch2)
{
  ...
  // pthread_mutex_lock(&branch2->lock);
  // for(int i = 0; i < branch2->numberAccounts; i++){
  //   pthread_mutex_lock(&branch2->accounts[i].lock);
  // }
  ...
  // for(int i = 0; i < branch2->numberAccounts; i++){
  //   pthread_mutex_unlock(&branch2->accounts[i].lock);
  // }
  // pthread_mutex_unlock(&branch2->lock);
  return err;
}
